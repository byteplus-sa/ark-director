...
def build_segments(cfg, work):
    ...
    for i, seg in enumerate(cfg["segments"]):
        ...
        if layout in ("split", "full_b"):
            clip = os.path.join(clips_dir, seg["clip"])
            trim = seg.get("crop_bottom_frac", 0)
            pre = f"crop=iw:ih*{1 - trim}:0:0," if trim else ""
            clip_in = ["-ss", f"{seg.get('offset', 0):.3f}", "-t", f"{dur:.3f}", "-i", clip]
            speed = seg.get("speed", 1)
            slow = f"setpts=PTS/{speed}," if speed != 1 else ""
        ...
        if layout == "full_m":
            ...
            run(clip_in + ["-vf", f"{pre}{slow}fps={FPS_EXPR},{cover(W, H, fx)},setsar=1"] + enc + [out])
        elif layout == "split":
            graph = (
                f"[0:v]{pre}{slow}fps={FPS_EXPR},{cover(W, HALF, fx)},setsar=1[t];"
...
def build_cues(cfg):
    ...
    for k, (a, b, text) in enumerate(chunks):
        ...
        if following and layout_class(following[0]["layout"]) != layout_class(seg["layout"]):
            en = min(en, seg["end"])
        cues.append({"text": text, "start": round(st, 3), "end": round(min(en, end), 3), "y": CAPTION_Y[layout_class(seg["layout"])]})
    return cues
...
def main():
    ...
    work = tempfile.mkdtemp(prefix="avatar_short_")
